void net_samplevarbin(){
   ...
   const Int_t n=7;
   Int_t pid[200];
   Double_t no_of[n],netchg[n],netchgsq[n],netchgvar[n],ntracktot[n],pT[200],eta[200],y[n];
   ...
   for(Int_t k=0; k<n; k++)
    ...
    if(no_of[k]>1)
      ...
      cout<<"("<<y[k]<<","<<netchgvar[k]<<")"<<endl;
      }
    }
   TGraph *gr3 = new TGraph(n,y,netchgvar); 
   TCanvas *c1 = new TCanvas("c1","Graph Draw Options",
                             200,10,600,400);
   c1->cd(4);
   gr3->SetTitle("Net Charge Variance vs Mean Multiplicity");
   ...
   for (Int_t j=0; j<n; j++) {
      TMarker *m = new TMarker(y[j], netchgvar[j], 22);
      m->SetMarkerSize(2);
      m->SetMarkerColor(2);
      m->Draw();
   }
